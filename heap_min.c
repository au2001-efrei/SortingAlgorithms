...
void heap_min_propagate_top(Heap *heap, int position) {
	...
	heap_min_propagate_top(heap, parent);
}
...
void heap_min_propagate_bottom(Heap *heap, int position) {
	...
	if (invert1 && (!invert2 || heap->data[child1] <= heap->data[child2])) {
		...
		heap_min_propagate_bottom(heap, child1);
	} else {
		heap->data[position] = heap->data[child2];
		heap->data[child2] = value;

		heap_min_propagate_bottom(heap, child2);
	}
}
...
bool heap_min_add(Heap *heap, int value) {
	...
	heap_min_propagate_top(heap, heap->size - 1);
	return true;
}
...
bool heap_min_pop(Heap *heap) {
	...
	if (heap->size > 0) {
		...
		heap->data[0] = value;
		heap_min_propagate_bottom(heap, 0);
	}
